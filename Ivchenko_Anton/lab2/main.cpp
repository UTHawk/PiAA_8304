...
void RECONSTRUCT_PATH(std::map<char, char>& from, char start, char end, std::ostream & out){
...
void A(Vertex start, Vertex end, int **m, int n, std::map<int, int> &cmp, std::ostream & out)
{
	...
	while (openset.size() != 0) {
		...
		for (auto a : openset)
			out << static_cast<char>(a.id) << ':' << a.F << endl;
		
		
		if (current.id == end.id) {
				RECONSTRUCT_PATH(fromset, start.id, end.id, out);
				return;
...
int main() {
	...
	if (chose == 0) {
		A(v_start, v_end, m, n, cmp, std::cout);
	}
	else {
		std::ofstream file;
